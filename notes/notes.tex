...
$V_{0}\delta t$, that is (eq. L9) $2\pi b{\rd}bV_{0}\delta t \fq(r,\bva){\rm d}^{3}\bva$.
...
\begin{array}{ccl}
  ...
  \langle\Delta v_{i}\Delta v_{j}\rangle & = &\displaystyle{ 4\pi\left(
    \frac{\ma}{m+\ma}\right)^{2}\int{\rd}^{3}\bva V_{0}^{3}b_{90}^{2}\fq(r,
...
\begin{equation}
  \begin{array}{ccl}
  h(r,\bv) & =&\displaystyle{\int_{0}^{\infty}{\rd} \va \va^{2} \int_{0}^{\pi} {\rd} \theta \sin \theta \int_{0}^{2\pi}{\rd}\phi \frac{\Fq(\Ea,\La)}{\sqrt{v^{2}+\va^{2}-2v \va \cos(\theta)}}} , \\

  {} & =&\displaystyle{\sum_{l=0}^{\infty}\int_{0}^{\infty}{\rd} \va \va^{2} \frac{\vinf^{l}}{\vsup^{l+1}}\int_{0}^{\pi} {\rd} \theta \sin\theta P_{l}(\cos\theta)  \int_{0}^{2\pi}{\rd}\phi \Fq(\Ea,\La)} , 
  \end{array}
  \label{eq:h_Legendre}
\end{equation}
...
\begin{equation}
\begin{array}{ccl}
  \displaystyle{\frac{\partial}{\partial \vr}\left[\fq(r,\bv-\bV0)\right]} & =&\displaystyle{\left(-\vr+V_{0}\cos\theta\right)\frac{\partial \Fq}{\partial E}} ,\\
  
   \displaystyle{\frac{\partial}{\partial \vt}\left[\fq(r,\bv-\bV0)\right]} & =&\displaystyle{\left(-\vt+V_{0}\sin\theta\cos\phi\right)\left(\frac{\partial \Fq}{\partial E}-\frac{r}{\La}\frac{\partial \Fq}{\partial L}\right)} ,\\
 
  \displaystyle{\frac{\partial^{2}}{\partial \vr^{2}}\left[\fq(r,\bv-\bV0)\right]} & =&\displaystyle{-\frac{\partial \Fq}{\partial E}+\left(-\vr+V_{0}\cos\theta\right)^{2}\frac{\partial^{2}\Fq}{\partial E{}^{2}}} ,\\
 
  \displaystyle{\frac{\partial^{2}}{\partial \vt\partial \vr}\left[\fq(r,\bv-\bV0)\right]} & =&\displaystyle{\left(-\vr+V_{0}\cos\theta\right)\left(-\vt+V_{0}\sin\theta\cos\phi\right)\left(\frac{\partial^{2}\Fq}{\partial E{}^{2}}-\frac{r}{\La}\frac{\partial^{2}\Fq}{\partial L\partial E}\right)} ,\\
 
  \displaystyle{\frac{\partial^{2}}{\partial \vt^{2}}\left[\fq(r,\bv-\bV0)\right]} & =&\displaystyle{-\frac{\partial \Fq}{\partial E}+\frac{r}{\La}\frac{\partial \Fq}{\partial L}+\left(-\vt+V_{0}\sin\theta\cos\phi\right)^{2}} \\
  {}&\times& \displaystyle{\left(\frac{\partial^{2}\Fq}{\partial E{}^{2}}-\frac{2r}{\La}\frac{\partial^{2}\Fq}{\partial L\partial E}-\frac{r^{2}}{\La^{3}}\frac{\partial \Fq}{\partial L}+\frac{r^{2}}{\La^{2}}\frac{\partial^{2}\Fq}{\partial L{}^{2}}\right)} .
\end{array}
...
where $\Fq(E,L)=F_{0}(E)$, i.e. $\fq(r,\bv)=f_{0}(r,v)=F_{0}(E)$.
...
\begin{align*}
 D_{r} &= \frac{\partial \Jr}{\partial E} \dE + \frac{\partial \Jr}{\partial L} \dL+\frac{1}{2}\bigg(\frac{\partial^2 \Jr}{\partial E^2} \dEE +2 \frac{\partial^2 \Jr}{\partial E \partial L} \dEL +\frac{\partial^2 \Jr}{\partial L^2} \dLL\bigg) \\
    &= -\frac{T}{2\pi} \dE -\frac{\Theta}{2\pi} \dL+\frac{1}{2}\bigg(-\frac{1}{2\pi}\frac{\partial T}{\partial E} \dEE -2 \frac{1}{2\pi} \frac{\partial T}{\partial L} \dEL -\frac{1}{2\pi}\frac{\partial \Theta}{\partial L} \dLL\bigg) \\
  D_{rr} &=\bigg(\frac{\partial \Jr}{\partial E} \bigg)^{2} \dEE + 2 \frac{\partial \Jr}{\partial E} \frac{\partial \Jr}{\partial L} \dEL + \bigg(\frac{\partial \Jr}{\partial L} \bigg)^{2} \dLL\\
  &=\bigg(\frac{T}{2\pi} \bigg)^{2} \dEE + 2 \frac{T}{2\pi}\frac{\Theta}{2\pi} \dEL + \bigg(\frac{\Theta}{2\pi} \bigg)^{2} \dLL \\
    D_{rL} &=\frac{\partial \Jr}{\partial E} \frac{\partial L}{\partial E} \dEE + 2 \frac{\partial \Jr}{\partial E} \frac{\partial L}{\partial L} \dEL + \frac{\partial \Jr}{\partial L} \frac{\partial L}{\partial L} \dLL\\
  &= -2 \frac{T}{2\pi} \dEL -\frac{\Theta}{2\pi}  \dLL \\
  \label{eq:changeVar}
\end{align*}

Using Hénon's method, we can get rid of the singularities in the integrand of $T$ and $\Theta$, as well as the $(E,L)$ dependencies in the integration bounds. After computation, we obtain the following expressions
\begin{align}
  T(E,L) &= \frac{1}{2}\int_{-\pi/2}^{\pi/2}\partial_{z} f(\ell\sin\phi,E)\rd\phi \\
  \Theta(E,L) &= \int_{-\pi/2}^{\pi/2} \frac{\partial_{z}f(\ell\sin\phi,E)}{f(\ell\sin\phi,E)}\rd\phi
  \end{align}

where $\ell^{2}(E) = \Lc^{2}(E) - L^{2}$ and $x=f(z,E)>0$ is the solution of the equation
\begin{equation}
  z^{2} = \zeta(x,E) = x \psi(\sqrt{x/2}) + E x - \Lc^{2},
  \label{eq:f(z)}
\end{equation}
chosen so that $x<\xc$ if $z<0$ and $x>\xc$ if $z>0$, where $\xc = 2\rrc^{2}$ correspond to the ''circular radius''. The function $z\mapsto f(z,E)$ is $C^{1}$ is the inverse bijection of $x\mapsto z(x,E)$, which is defined by

\begin{equation}
z(x,E)=\begin{cases}
\displaystyle{-\sqrt{\zeta(x,E)}} & {\rm{if}} \quad x\in [x_{\min},\xc] ,\\
\displaystyle{+\sqrt{\zeta(x,E)}} & {\rm{if}} \quad x\in [\xc,x_{\max}] .
\end{cases}
\label{eq:z(x)}
\end{equation}
...
\begin{equation}
\partial_{x}z(x,E)=\begin{cases}
\displaystyle{\frac{\partial_{x}\zeta(x,E)}{2 z(x,E)}} & {\rm{if}} \quad x\neq \xc ,\\
&\\
\displaystyle{\sqrt{\frac{\partial_{xx}^{2}\zeta(\xc,E)}{2}}} & {\rm{if}} \quad x = \xc ,
\end{cases}
\label{eq:dzdx}
\end{equation}

where $\partial_{xx}^{2}\zeta(x,E)=(x+8)/(2\sqrt{2}(x+2)^{5/2})>0$. Therefore we have (with the correspondance $x\leftrightarrow z$)

\begin{equation}
\partial_{z}f(z,E)=\begin{cases}
\displaystyle{\frac{2 z}{\partial_{x}\zeta(f(z),E)}} & {\rm{if}} \quad z\neq 0 ,\\
&\\
\displaystyle{\sqrt{\frac{2}{\partial_{xx}^{2}\zeta(\xc,E)}}} & {\rm{if}} \quad z = 0 ,
\end{cases}
\label{eq:dfdz}
\end{equation}
...
\section{Velocity-derivatives of $\fq$}
