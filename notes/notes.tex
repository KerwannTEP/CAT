...
\begin{equation}
L_{{\rm c}}^{2}(E)=\eta(r_{*}^{E};E)=\max_{r>0}\eta(r;E).
\label{eq:angular_circular}
\end{equation}
where we defined $\eta(r;E)\doteqdot 2 r^{2}(E-\psi(r))$. Its derivatives are


\begin{equation}
\begin{array}{ccl}
  \eta'(r;E) & =& \displaystyle{4r(E-\psi(r))-2r^{2}\psi'(r)} ,\\
  
  \eta''(r;E) & =&\displaystyle{4(E-\psi(r))-8r\psi'(r)-2r^{2}\psi''(r)} .
\end{array}
...
the equation becomes $a x^{2}+b x^{2}+c x+d = 0$. Setting $x=t-b/(3a)$ and
\begin{align*}
  p &= (3ac -b^{2})/(3a^{2}), \\
